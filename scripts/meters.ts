...
const expect = (what: string, got: number | string, want: number | string, tol = 0) => {
  ...
  console.log(`  ${ok ? "✓" : "✗"} ${what.padEnd(46)} ${fmt(got).padStart(12)}  expected ${fmt(want)}${tol ? ` ±${tol}` : ""}`)
}
...
console.log("Voltmeter across the lower leg of a 1 k / 2 k divider on 9 V")
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "9 V" })
  ...
  const v = meterValue(snap, pv.id, "voltage")
  expect("reads the divider output 9·2/3 (V)", v, 6.0, 0.02)
  const i = snap.pinCurrent[pinKey(pv.id, "+")]
  expect("its 10 MΩ input barely loads it (µA)", Math.abs(i!) * 1e6, 0.6, 0.05)
}

console.log("\nAmmeter in series with a 470 Ω load on 5 V")
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "5 V" })
  ...
  wire(bat, "-", gnd, "GND")
  const { snap } = settle(doc)
  const i = meterValue(snap, pa.id, "current")
  expect("reads 5 V / 470 Ω (mA)", Math.abs(i) * 1e3, 10.638, 0.05)
  const across = snap.readings.find((x) => x.object === pa.id && x.element === 0)!
  expect("burden voltage of the 0.01 Ω shunt (mV)", Math.abs(across.voltage) * 1e3, 0.106, 0.01)
}

console.log("\nBoth on a 5 V RMS, 50 Hz sine into a 1 kΩ load: RMS")
{
  const { doc, place, wire } = builder(GRID)
  ...
  expect("AC circuit detected", snap.ac ? "yes" : "no", "yes")
  expect("voltmeter reads the 5 V RMS across the load", meterValue(snap, pv.id, "voltage"), 5.0, 0.05)
  expect("ammeter reads 5 mA RMS", Math.abs(meterValue(snap, pa.id, "current")) * 1e3, 5.0, 0.05)
}

console.log("\nField readout before the run is a dash")
{
  ...
  const def = getDef("voltmeter")!
  expect("voltmeter has a meter spec", def.meter?.read ?? "none", "voltage")
  expect("ammeter reads current", getDef("ammeter")!.meter?.read ?? "none", "current")
}
