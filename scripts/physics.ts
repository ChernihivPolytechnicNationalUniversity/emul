...
console.log("A coil let go: 12 V through a switch into 100 mH + 100 Ω, then the switch opens")
{
  ...
  const bat = place("battery", 0, 0, { value: "12 V", imax: "5 A" })
...
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "5 V", imax: "10 A" })
...
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "3 V", imax: "10 A" })
...
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "12 V" })
...
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "800 V" })
...
{
  const { doc, place, wire } = builder(GRID)
  const bat = place("battery", 0, 0, { value: "230 V", imax: "10 A" })
  const tr = place("transformer", 6, 0)
  const load = place("resistor", 14, 0, { value: "100 Ω", power: "5" })
  const gnd = place("ground", 3, 8)
  wire(bat, "+", tr, "P1")
...
console.log("\n12 V on a Nucleo pin: the chip dies and its supply shorts")
{
  ...
  const bat = place("battery", 40, 10, { value: "12 V", imax: "3 A" })
  wire(bat, "+", u, "CN7-10")
  wire(bat, "-", u, "CN7-8")
  const t = start(doc)
  const snap = t.run(0.05)
